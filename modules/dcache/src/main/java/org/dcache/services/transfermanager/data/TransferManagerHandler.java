...
public class TransferManagerHandler extends AbstractMessageCallback<Message>
{
                                  ...
                                  Executor executor)
    {
        ...
        try {
            ...
            LOGGER.error("starting tlog failed :", e);
        }
    ...
    public Object appendInfo(final TransferStatusQueryMessage message)
    {
        ...
        final ListenableFuture<IoJobInfo> future = manager.getPool().
    ...
    public void deletePnfsEntry()
    {
        if (state == RECEIVED_PNFS_CHECK_BEFORE_DELETE_STATE) {
            ...
            CellStub.addCallback(manager.getPnfsManager().send(pnfsMsg), this, executor);
        } else {
            PnfsMapPathMessage message = new PnfsMapPathMessage(pnfsPath);
            setState(WAITING_FOR_PNFS_CHECK_BEFORE_DELETE_STATE);
            CellStub.addCallback(manager.getPnfsManager().send(message), this, executor);
        }
    }
    ...
    public void handle()
    {
        ...
        String parentDir = pnfsPath.substring(0, last_slash_pos);
    ...
    public void killMover(int moverId)
    {
        ...
        try {
            manager.getPool().notify(new CellPath(poolAddress), killMessage);
        } catch (NoRouteToCellException e) {
            LOGGER.error("failed to kill mover {} on pool={}: {}", moverId, pool,
                    e.toString());
        }
    }
    ...
    public void poolFirstReplyArrived(PoolIoFileMessage poolMessage)
    {
        ...
        LOGGER.debug("Pool " + pool + " will deliver file " + pnfsId + " mover id is " + poolMessage.getMoverId());
    ...
    public void selectPool()
    {
        ...
        manager.persist(this);
        CellStub.addCallback(manager.getPoolManager().send(request), this, executor);
    }
    ...
    public void sendErrorReply()
    {
        ...
        try {
            ...
            manager.getEndpoint().sendMessage(new CellMessage(requestor, errorReply));
        } catch (Exception e) {
    ...
    public void setState(int istate, Serializable errorObject)
    {
        this.state = istate;
    ...
    public void startMoverOnThePool()
    {
        ...
        CellStub.addCallback(manager.getPool().send(poolCellPath, poolMessage), this, executor);
    }
    ...
    public void success(Message message)
    {
            if (message instanceof PnfsCreateEntryMessage) {
                ...
                if (state == WAITING_FOR_PNFS_CHECK_BEFORE_DELETE_STATE) {
                    ...
                    LOGGER.error(this.toString() + " got unexpected PnfsMapPathMessage "
                            + " : " + mapMessage + " ; Ignoring");
                }
    ...
    void sendDoorRequestInfo(int code, String msg)
    {
        try {
            ...
            manager.getBilling().notify(info);
        } catch (NoRouteToCellException e) {
            LOGGER.error("Couldn't send billing info", e);
        }
    }
}
