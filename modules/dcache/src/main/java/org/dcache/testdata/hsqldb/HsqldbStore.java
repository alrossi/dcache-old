package org.dcache.testdata.hsqldb;
...
public class HsqldbStore extends Store {
    ...
    private static final String url = "jdbc:hsqldb:mem:nucleus1";
    ...
    public HsqldbStore(HikariConfig config) throws IOException {
        super(url);
    ...
    public HsqldbStore(HikariConfig config, String connectorName)
                    throws IOException {
        super(url);
        config.setJdbcUrl(url);
        config.setUsername(user);
    ...
    protected void setOptionalProperties(Properties properties) {
        ...
        properties.setProperty("javax.jdo.option.Mapping", "hsql");
