...
public class PoolV2Mode implements Serializable {
    ...
    private static final int REPLICATION_ENABLED   = 0x7F ;
    private static final int REPLICATION_DISABLED  = 0x80 ;
     ...
     *  For convenience.
     */
    public synchronized boolean isReplicationEnabled() {
        return !((_mode & REPLICATION_DISABLED) == REPLICATION_DISABLED);
    }

    public synchronized void setReplicationEnabled(boolean isReplicationEnabled) {
        if (isReplicationEnabled) {
            _mode &= REPLICATION_ENABLED;
        } else {
            _mode |= REPLICATION_DISABLED;
        }
    }
    ...
    private static boolean isEnabled(int mode) {
        return mode == ENABLED || mode == (ENABLED|REPLICATION_DISABLED);
    }
}
